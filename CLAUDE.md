...
- **Two Lambdas split by network requirement.** The executor stays inside
  the VPC talking to Aurora over a private security group; everything that
  needs public AWS APIs runs outside it. This kept the production-posture
  networking story while removing the NAT gateway (ADR-013). The RDS Data
  API would have been the simpler fix but is not offered for Aurora
  PostgreSQL in us-east-2 -- verified three ways, recorded in ADR-013.
...
Browser
  ...
  -> API Gateway HTTP API (CORS scoped to CloudFront domain)
        -> API Lambda  (Python 3.12, arm64, OUTSIDE the VPC)
              -> SSM Parameter Store (model ID, cluster id, table name)
              -> Bedrock Runtime (Converse -- intent parser + summary writer)
              -> CloudWatch (ServerlessDatabaseCapacity series)
              -> DynamoDB (run header + per-second metric rows)
              -> async invoke:
                 Executor Lambda (Python 3.12, arm64, IN the VPC, NO internet)
                       -> Aurora Serverless v2 (private IP, IAM auth token)
                       -> DynamoDB (free gateway endpoint)

VPC: 2 AZs, private subnets only. No IGW, no NAT, no public subnets.
Aurora SG: ingress 5432 from the executor SG only.
Executor SG: egress 5432 to Aurora SG only -- no route to the internet at all.

Why the split (ADR-013): an in-VPC Lambda needed a NAT gateway to reach
Bedrock/SSM/Secrets Manager, and NAT + its Elastic IP were $36/mo of a
measured ~$82/mo bill, billed hourly regardless of traffic. Separating the internet-facing
work from the database work removed the NAT entirely. The executor
authenticates with IAM database authentication, whose token is generated by
local SigV4 signing (no network call) -- that is what allows a Lambda with
zero egress to still authenticate to Postgres.

## Non-negotiable rules

- **Never** commit secrets, AWS keys, DB passwords, or hardcoded ARNs/model
  IDs. Config values live in SSM Parameter Store and reach Lambda as env
  vars resolved at deploy time; runtime DB auth is a locally-signed IAM
  token (ADR-013), and the Secrets-Manager master secret is break-glass
  only.
...
- **CI never holds AWS credentials** (ADR-010). Pipelines run lint,
  tests, `terraform fmt`/`validate` and checkov only; deploys are a
  local `terraform apply`. v1.5 adds a GitHub OIDC role for plan-on-PR
  and gated applies.
...
- Module layout under `app/src/ngx_workload_lab/`:
  ...
  - `config.py` — cold-start env loader (values resolved from SSM at deploy time)
...
- ~~**IAM auth for Postgres.**~~ Shipped in ADR-013. The executor connects
  ...
  role and removes break-glass access.
